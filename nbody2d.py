...
c = {"npart": 128, "boxsize": 100., "drawsteps": 100, "framepause": 1e-4, "pmgrid":128, "outdir":None, "aic":0.05, "afin":1.0, "da":0.01, "aspectratio":1.5, "width": 10.0, "no_overlay": False, "vismode": "point", "pointsize":2.0, "verbose":False}
kwargs = vars(args)
for key in kwargs:
...
for i in range(c["drawsteps"]):
    ...
    if c["verbose"]:
        print("frame %d, a=%.2f, dtintegrate = %.0fms dtdraw = %.0fms dtpause/dtdisplay = %.0fms" % (i, sim.a, 1000.*(t1-t0), 1000.*(t2-t1), 1000.*(t3-t2)))
