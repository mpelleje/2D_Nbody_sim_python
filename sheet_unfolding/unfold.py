...
def unfold3d(pos, L, mass=1., mode=None, simp=None, idptr=None, output_flat=True):
    assert pos.shape[-1] == 3
    
    pos_in = np.float32(pos.reshape(-1,3))
    mass_in = np.ones(len(pos_in), dtype=np.float32) * np.reshape(np.float32(mass), -1)
    
    ngrid = np.int64(np.cbrt(pos_in.shape[0]))
    
    if mode is None:
        mode = 4
    
    if simp is None:
        simp_in = math.tesselation3d(ngrid)
    else:
        simp_in = np.int64(simp)
...
def multistep_unfold3d(pos_array, L, mass=1., mode=None, simp=None, idptr=None, output_flat=True):
    """pos_array: an array of particle positions at different times, e.g. shape [nsnap, 128, 128, 2]
    other parmeters are like in unfold2d
    """
    shape = pos_array[0].shape[:-1]
    
    pos = pos_array[0].reshape(-1,3)
    mass0 =  np.ones(len(pos), dtype=np.float32) * np.reshape(np.float32(mass), -1)
    
    # Note that the only variables that accumulate changes in this loop are idptr and tri
    for pos in pos_array:
        if idptr is not None:
            pos, mass = math.stick_through_idptr(idptr, pos.reshape(-1,3), mass=mass0, L=L)
        
        pos, mass, simp, idptr = unfold3d(pos, L, mass=mass, mode=mode, simp=simp, idptr=idptr, output_flat=True)
        
        while np.any(idptr[idptr] != idptr):
            idptr = idptr[idptr]
        
    if not output_flat:
        return pos.reshape(shape + (3,)), mass.reshape(shape), simp, idptr.reshape(shape)
    else:
        return pos, mass, simp, idptr